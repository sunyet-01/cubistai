...
// NOTE: import.meta.url is URL-encoded on non-ASCII paths (e.g. Chinese
// folder names) — decode it or the alias points at %E7%BC%96... and the
// Cloudflare build fails to load the stub.
const driverStub = decodeURIComponent(
  new URL('./src/core/db/driver-stub.ts', import.meta.url).pathname
);
  ...
  plugins: [
    ...
    paraglideVitePlugin({
      ...
      urlPatterns: [
        ...
        // "as-needed" prefix: zh under /zh, zh-tw under /zh-tw, ja under /ja,
        // ko under /ko, en unprefixed.
        {
          pattern: '/:path(.*)?',
          localized: [
            ['zh', '/zh/:path(.*)?'],
